...
本阶段实现了可独立运行的 Modbus TCP 模拟链路：

```text
MainWindow
  -> QProcess 启动 upkun-simulator.exe
  -> upkun-simulator.exe 内部启动 SimulatedModbusServer
  -> LineSimulator 维护虚拟 PLC 点位和产线状态
  -> ModbusTcpClient 通过 127.0.0.1:1502 轮询点位
...
- 模拟器页可通过 Modbus TCP 触发急停、气压不足、缺瓶、堵瓶、缺盖、重量不合格、缺标签、剔除失败、下料满料等故障，也可清除模拟报警。
...
| `src/simulator/main.cpp` | 独立模拟器进程入口 |
| `src/app/MainWindow.*` | 启动模拟器进程、连接客户端、桥接 UI |
