...
- 当前阶段：M14 模拟器拆分已进入实现，上位机启动独立模拟器进程，并通过 Modbus TCP 完成状态采集、控制和故障注入。
...
- [M13 模拟器增强说明](docs/16-simulator-enhancement.md)
- [M14 模拟器拆分说明](docs/17-simulator-split.md)

## 当前实现能力

- Qt Widgets 主窗口、主监控页和模拟器页。
- 独立 Modbus TCP 模拟器进程 `upkun-simulator.exe`，默认监听 `127.0.0.1:1502`。
- Modbus TCP 客户端自动连接模拟器并轮询状态。
- 主监控页可发送启动、停止、复位、报警确认命令。
- 模拟器页可查看传感器/执行机构状态，触发急停、气压不足、缺瓶、堵瓶、缺盖、重量不合格、缺标签、剔除失败、下料满料等故障并清除故障。
- 报警记录页可查看最近报警和最近操作日志。
...
- M13 新增模拟器速度扰动、质量波动、随机不良、剔除测试和命名化传感器点位。
- M14 新增独立模拟器可执行文件，上位机和模拟器之间只通过 Modbus TCP 交互。
